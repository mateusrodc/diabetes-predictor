...
aba = st.sidebar.radio("Navegação", ["🔍 Previsão", "📜 Histórico completo"])
...
if aba == "🔍 Previsão":
    ...
    if registros:
        df = pd.DataFrame([{
            ...
            "Idade": r.idade,
            "IMC": r.imc,
            "Probabilidade": f"{r.probabilidade:.2%}",
            "Resultado": r.resultado
        } for r in registros])
        st.dataframe(df, use_container_width=True)
    else:
        st.info("Nenhuma previsão registrada no banco ainda.")